...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    h = np.matmul(outside_vectors, center_word_vec)
    y_cap = softmax(np.matmul(outside_vectors, center_word_vec))
    y = np.zeros(y_cap.shape)
    y[outside_word_idx] = 1

    loss = -1*np.dot(y, np.log(y_cap))
    grad_center_vec = np.matmul(outside_vectors.T, (y_cap - y).reshape([len(y), -1]))
    grad_center_vec = np.squeeze(grad_center_vec)
    grad_outside_vecs = np.matmul((y_cap-y).reshape([len(y), -1]), (center_word_vec.reshape([len(center_word_vec), -1])).T)
    ### END YOUR CODE
...
def skipgram(current_center_word, window_size, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        ...
        loss_cw, grad_center_vec_cw, grad_outside_vecs_cw = naive_softmax_loss_and_gradient(center_word_vectors[current_center_word_idx,:], outside_word_idx, outside_vectors, dataset)
        loss += loss_cw
        grad_center_vecs[current_center_word_idx] += grad_center_vec_cw
        grad_outside_vectors += grad_outside_vecs_cw
    ### END YOUR CODE
